...
function compareObjects(obj1, obj2) {
  ...
  if (keys1.length !== keys2.length) return false; // Разное число ключей → false
...
function makeWord(lettersObject) {
  ...
  const maxLength = allPositions.length > 0 ? Math.max(...allPositions) + 1 : 0;
  const wordArray = new Array(maxLength).fill('');
